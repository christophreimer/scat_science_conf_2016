...
parser.add_argument("template", help="path to the reveal.js template to be used")
parser.add_argument("revealjs", help="path to the reveal.js framework")
...
if args.template[-5:] != '.html':
    print(' ')
    print("You gave me a template without a .html extension.  Appending to file.")
    thetemplate = args.template + '.html'
else:
    thetemplate = args.template
if not isfile(thetemplate):
    raise IOError("Template %s file not found" % template)

# set path to revealjs framework
therevealjspath = args.revealjs
...
commandstring = 'pandoc -t html5 --template="%s" --standalone --section-divs --variable revealjs="%s" --variable theme="%s"  --variable transition="%s" %s -o %s' % (thetemplate, therevealjspath, thetheme, thetransition, theinfile, theoutfile)
print(' ')
